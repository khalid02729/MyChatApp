...
let socket = null, currentUser = null, activeChatUser = null, activeChats = {}, globalAvatarBase64 = "", storyMediaBase64 = "", storyMediaType = "text";

window.onload = function() {
    try {
        const u = localStorage.getItem("chat_user");
        if (u) { currentUser = JSON.parse(u); showChatScreen(); }
    } catch (e) {}
};
...
async function loadActiveChatsFromServer() {
    try {
        const r = await fetch(`${SERVER_URL}/api/active-chats?username=${currentUser.username}`), c = await r.json();
        const container = document.getElementById("chats-list-container"); if(!container) return; container.innerHTML = "";
...
async function fetchActiveChatMessages() {
    ...
    try {
        const r = await fetch(`${SERVER_URL}/api/messages?sender=${currentUser.username}&receiver=${activeChatUser.username}`), m = await r.json();
        const box = document.getElementById("messages-box"); if(!box) return; box.innerHTML = "";
        m.forEach(msg => {
            const isMe = msg.sender_username === currentUser.username;
            const bubble = document.createElement("div"); bubble.className = `msg-bubble ${isMe ? 'sent' : 'received'}`; bubble.innerText = msg.message;
            if (!msg.deleted_for_all && isMe) { bubble.style.cursor = "pointer"; bubble.onclick = () => { if(confirm("هل تريد حذف هذه الرسالة لدى الجميع؟")) triggerDeleteMessage(msg.id); }; }
...
async function triggerDeleteMessage(id) {
    try { await fetch(`${SERVER_URL}/api/delete-message`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: id, sender_username: currentUser.username, receiver_username: activeChatUser.username }) }); fetchActiveChatMessages(); } catch(e){}
}

async function sendMessage() {
    const input = document.getElementById("message-input"); if(!input) return;
    const txt = input.value.trim(); if (!txt || !activeChatUser) return; input.value = ""; 
