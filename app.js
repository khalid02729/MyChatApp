const BASE_URL = https://mychatapp-production-b225.up.railway.app
...
function sendMessage() {
    ...
    fetch(`${BASE_URL}/send?sender=${currentSender}&receiver=${currentReceiver}&message=${encodeURIComponent(text)}`)
...
function searchUser() {
    ...
    fetch(`${BASE_URL}/search?phone=` + encodeURIComponent(phone))
        .then(response => response.text())
        .then(result => {
            alert(result);
...
function loadMessages() {
    ...
    fetch(`${BASE_URL}/get_messages?sender=${currentSender}&receiver=${currentReceiver}`)
