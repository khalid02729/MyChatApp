...
def register():
    ...
    try:
        with get_db() as conn:
            conn.execute('INSERT INTO users (username, phone, password) VALUES (?, ?, ?)', (username, phone, hashed_password))
            conn.commit()
        return jsonify({'status': 'success', 'message': 'تم تسجيل الحساب بنجاح'})
    except sqlite3.IntegrityError:
        return jsonify({'status': 'error', 'message': 'هذا الرقم مسجل بالفعل'}), 400
